enum Affine {
    struct Parameter {
        ...
        init(scale: Int = 10, angle: Int = 0) {
            let cos = cosTable[angle] * Int16(scale) / 10
            let sin = sinTable[angle] * Int16(scale) / 10
            self.a = UInt16(bitPattern: cos)
            self.b = UInt16(bitPattern: sin)
            self.c = UInt16(bitPattern: sin * -1)
            self.d = UInt16(bitPattern: cos)
        }
    }
    
    static let sinTable: [Int16] = [
